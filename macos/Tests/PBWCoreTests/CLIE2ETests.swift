...
final class CLIE2ETests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempHome = FileManager.default.temporaryDirectory
            .appendingPathComponent("pbw-cli-e2e-\(UUID().uuidString)", isDirectory: true)
    ...
    func testCLIStableEnvelopeExitCodesAndPolicy() throws {
        ...
        let destructiveResult = try runPBW(["clipboard", "clear"])
        XCTAssertEqual(destructiveResult.exitCode, 2)
        assertEnvelope(destructiveResult.json, ok: false, code: "confirmation_required")
    }

    private func runPBW(_ arguments: [String]) throws -> (exitCode: Int32, json: [String: Any]) {
        guard let binary = ProcessInfo.processInfo.environment["PBW_BIN"], !binary.isEmpty else {
            throw XCTSkip("PBW_BIN is not set; skipping process-level CLI E2E.")
        }
        ...
        environment["PBW_HOME"] = tempHome.path
        ...
        guard let json = try JSONSerialization.jsonObject(with: output) as? [String: Any] else {
            ...
            XCTFail("pbw did not return a JSON object: \(text)")
